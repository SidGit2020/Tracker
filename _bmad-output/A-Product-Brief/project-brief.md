# Product Brief: Tracker

## Strategic Summary

Tracker is a dead-simple expense tracker that turns "I probably spend too much on X" into an actual monthly number per category. It's built for an audience of one — the founder himself — with manual entry only and zero setup friction. The problem it solves isn't blindness to spending; it's vagueness. The founder already senses he's overspending somewhere, and Tracker's job is to convert that feeling into a specific, trustworthy number, checked in on roughly monthly.

Where automation-heavy alternatives (Walnut, ETMoney) solve this by demanding bank-linking and broad permissions, and existing manual-entry apps (Monefy, Goodbudget, Pocket Clear) solve it but aren't owned by the person using them, Tracker's differentiation is simple: it's built and owned by its one user, free to evolve on his own terms rather than someone else's roadmap.

Success isn't measured by growth or revenue — there is none planned. It's measured by whether the founder is still actively logging expenses two weeks after launch, which is the real test of whether the core abandonment risk (forgetting to log in the moment) was solved by the product's design.

## Vision

A dead-simple expense tracker that turns "I probably spend too much on X" into an actual monthly number per category — manual entry only, zero setup friction, built for an audience of one: the founder himself.
...
**Product Concept (how this shapes the build):**
The home screen is a single always-visible quick-add box — amount field plus single-tap category buttons for the fixed, short category list — paired with a live running current-month total that updates immediately per entry. Every downstream decision should protect the speed of the add-flow first: no required navigation, no multi-step forms, no mandatory fields beyond amount + category. This directly counters the abandonment risk named above.
...
**Primary Metric (User Behavior):** Still actively logging expenses at day 14 post-launch — the exact threshold named as the make-or-break signal for the guilt/abandonment risk. This is what v1's launch is actually meant to test.

**Secondary Metric (Timeline):** Ship v1 within the self-imposed weekend-scale ("days") build window. If this slips into weeks/months, the "genuine discipline, not avoidance" verdict on bare-v1-first sequencing should be revisited, not assumed to still hold.
...
- **Timeline:** Weekend-scale build target — flexible if it slips, but a slip is a signal worth reflecting on, not a hard failure.
...
- **Offline Functionality:** Not required for v1 — assumes connectivity at logging time; candidate fast-follow, not committed
...
2. **Neutral, non-judgmental** — Never scolds or nudges about spending amounts; a large category total is just a number, not a verdict. Directly protects against the guilt/abandonment risk without needing a "friend" personality.
