...
- **FR-4 — Entry list.** Logged expenses for the current period are visible on the Home screen, each showing amount, category, and date logged (date only, not time-of-day). `[RESOLVED]` Confirmed during implementation readiness review (2026-07-05): v1 needs date, not time-of-day — matches the drill-down panel's existing date-only display (FR-8) and avoids exposing a granularity the product doesn't need.
