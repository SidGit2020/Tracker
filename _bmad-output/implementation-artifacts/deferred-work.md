...
## Deferred from: code review of 2-1-monthly-breakdown (2026-07-06)

- `GetMonthlySummary` repeats the same full-`Entries`-table-into-memory trade-off as `GetEntries` (see above) — plus derives `earliestYear`/`earliestMonth` via an in-process `Min()` over that same full list rather than a targeted query, and neither query uses `AsNoTracking()` despite being read-only. Accepted as consistent with the established v1 trade-off; revisit together if entry counts ever grow large enough to matter. [server/Entries/EntriesEndpoints.cs — `GetMonthlySummary`]
- If `AsNoTracking()` is ever added to these queries, the category grouping (`GroupBy(e => e.Category)`) must be changed to group by `e.CategoryId` first — grouping by the entity reference only produces correct results today because EF's change tracker (no `AsNoTracking`) deduplicates same-PK instances via its identity map. [server/Entries/EntriesEndpoints.cs — `GetMonthlySummary`]
