...
| Content | `{amount}` · `{category}` · `{date}` — date only (e.g. "Jun 28"), not time-of-day; matches the drill-down panel's date-only convention. `[RESOLVED 2026-07-05]` Added during implementation readiness review — PRD FR-4 originally flagged this as an open assumption. |
