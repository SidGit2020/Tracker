...
```
+---------------------------------------------------------+
| Monthly Category Breakdown          [ ‹ June 2026 › ]   |
+---------------------------------------------------------+
...
|----------|---------|--------|
| Layout | Header row (title + selector inline) | Header stacked: title, then month selector below |
...
| Property | Value |
|----------|-------|
| Component | Bar chart row (label + bar + amount) |
| Content | `{category}`, bar width proportional to `{amount}` relative to the largest category, `{amount}` label |
| Behavior | onClick opens the 2.2 drill-down panel for that category |
